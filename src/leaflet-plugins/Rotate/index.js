...
(function (factory) {
    ...
    L.Popup.include({
        ...
        // 🌟 1. 补上缺失的旋转事件监听
        getEvents: function() {
            var events = popupProto.getEvents.apply(this, arguments);
            events.rotate = this._updatePosition;
            return events;
        },

        // 🌟 2. 彻底重写 Popup 的位置更新逻辑
        _updatePosition: function () {
            if (!this._map) { return; }

            // 如果地图没有开启旋转，老老实实走原生逻辑
            if (!this._map._rotate) {
                return popupProto._updatePosition.apply(this, arguments);
            }
            ...
            var offset = L.point(this.options.offset || [0, 7]);
            ...
            // 接管底部尖角对齐
            var width = this._containerWidth || this._container.offsetWidth || 0;
            var bottom = this._containerBottom = -anchor.y - offset.y;
            var left = this._containerLeft = -Math.round(width / 2) + anchor.x + offset.x;
