...
export const Draw = L.Class.extend({
  ...
  _drawTrackLine: function (trackpoints) {
    ...
    let lineOptions = trackpoints[0].info
    ...
    if (options.stroke) {
      this._ctx.strokeStyle = lineOptions.color || options.color
      this._ctx.lineWidth = lineOptions.weight || options.weight
      this._ctx.stroke()
    }
    if (options.fill) {
      this._ctx.fillStyle = lineOptions.fillColor || options.fillColor
      this._ctx.fill()
    }
    this._ctx.restore()
  },
  ...
  _drawTrackPointsCanvas: function (trackpoints) {
    ...
    for (let i = 0, len = trackpoints.length; i < len; i++) {
      if (trackpoints[i].isOrigin) {
        ...
        this._ctx.globalAlpha = options.opacity
        if (options.stroke) {
          this._ctx.strokeStyle = trackpoints[i].pointColor || options.color
          this._ctx.stroke()
        }
        if (options.fill) {
          this._ctx.fillStyle = trackpoints[i].pointColor || options.fillColor
          this._ctx.fill()
        }
        if (trackpoints[i].labelText && trackpoints[i].showLabel !== false) {
          let prevOriginPoint = null
          let nextOriginPoint = null

          for (let p = i - 1; p >= 0; p--) {
            if (trackpoints[p]?.isOrigin) {
              prevOriginPoint = trackpoints[p]
              break
            }
          }

          for (let n = i + 1; n < len; n++) {
            if (trackpoints[n]?.isOrigin) {
              nextOriginPoint = trackpoints[n]
              break
            }
          }

          this._drawtxt(trackpoints[i].labelText, trackpoints[i], {
            direction: i % 2 === 0 ? 'left' : 'right',
            prevTrackPoint: prevOriginPoint,
            nextTrackPoint: nextOriginPoint
          })
        }
      }
    }
  ...
  _drawtxt: function (text, trackpoint, options = {}) {
    let point = this._getLayerPoint(trackpoint)
    const lines = Array.isArray(text) ? text : [text]
    const direction = options.direction === 'left' ? -1 : 1
    const prevTrackPoint = options.prevTrackPoint
    const nextTrackPoint = options.nextTrackPoint
    let startPoint = prevTrackPoint ? this._getLayerPoint(prevTrackPoint) : null
    let endPoint = nextTrackPoint ? this._getLayerPoint(nextTrackPoint) : null
    ...
    const anchorX = point.x + nx * 26
    const anchorY = point.y + ny * 26
    const textSide = options.direction === 'left' ? -1 : 1
    ...
    this._ctx.strokeStyle = 'rgba(0, 0, 0, 0.72)'
    this._ctx.lineWidth = 3
    this._ctx.textAlign = 'left'
    this._ctx.textBaseline = 'middle'
    this._ctx.beginPath()
    this._ctx.lineWidth = 1.5
    this._ctx.strokeStyle = 'rgba(125, 211, 252, 0.85)'
    this._ctx.moveTo(point.x, point.y)
    this._ctx.lineTo(anchorX, anchorY)
    this._ctx.stroke()
    this._ctx.translate(anchorX, anchorY)
    this._ctx.rotate(angle)
    this._ctx.lineWidth = 3
    this._ctx.strokeStyle = 'rgba(0, 0, 0, 0.72)'
    this._ctx.textAlign = textSide > 0 ? 'left' : 'right'
    for (let i = 0; i < lines.length; i++) {
      const line = String(lines[i] || '')
      const offsetX = textSide > 0 ? 8 : -8
      const offsetY = i * 16 - ((lines.length - 1) * 8)
      this._ctx.strokeText(line, offsetX, offsetY, 240)
      this._ctx.fillText(line, offsetX, offsetY, 240)
    }
    this._ctx.restore()
  },
  ...
  _getTooltipText: function (targetobj) {
    ...
    if (targetobj.info && targetobj.info.length) {
      for (let i = 0, len = targetobj.info.length; i < len; i++) {
        content.push('<tr>')
        content.push('<td style="padding-right: 10px;">' + targetobj.info[i].key + '</td>')
        content.push('<td style="padding-left: 6px;">' + targetobj.info[i].value + '</td>')
        content.push('</tr>')
      }
    }
    content.push('</table>')
